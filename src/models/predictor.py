"""
predictor.py — Five-output ensemble predictor.

Bug fixes:
  - is_home now passed in and applied to R/RBI sections
  - Uses updated pitcher_hit_factor (K%-based, not ERA)
  - Uses updated recent_form_factor (total bases, not HR count)
  - pitcher_hr_pa_rate now sourced from engineer.py (not duplicated)
"""
...
from src.features.engineer import (
    ...
    log5, park_factor, pitcher_hr_pa_rate, pitcher_hit_factor,
    platoon_adjustment, recent_form_factor, weather_factor,
    home_factor, expected_tb_tonight, expected_hrbi_tonight,
    _safe,
)
...
def _blend(xgb_rate, stat_rate) -> tuple[float, str]:
    if xgb_rate is not None:
        return 0.65 * xgb_rate + 0.35 * stat_rate, "xgb+stat"
...
def ensemble_predict(
    ...
    weather: dict,
    config: dict,
    is_home: bool = False,          # BUG FIX: was hardcoded to 1.0
    xgb_models: dict | None = None,
    xgb_metas:  dict | None = None,
    ...
    home_f  = home_factor(is_home)    # BUG FIX: now uses actual home/away

    # ── HR ────────────────────────────────────────────────────────────────
    stat_hr_rate, season_pa = hr_rate_from_stats(batter_season, batter_career)
    xgb_hr = _xgb("hr")
    base_hr, src_hr = _blend(xgb_hr, stat_hr_rate)

    p_hr_rate    = pitcher_hr_pa_rate(pitcher_season, pitcher_career)
    ...
    br   = statcast_metrics.get("barrel_pct")     # fixed: now barrel_pct not barrel_rate
    sc_prob, sc_conf = 0.15, 0.0
    if n_bb >= 30 and br is not None and br == br:
        ev = float(statcast_metrics.get("exit_velocity") or 88.3)
        hh = float(statcast_metrics.get("hard_hit_pct")  or 0.37)
        sc_score = 8.0*float(br) + 0.08*(ev-88) + 1.5*hh - 1.80
    ...
    xgb_tb = _xgb("tb")
    base_tb, src_tb = _blend(xgb_tb, stat_tb_rate)
    ...
    base_h,   src_h   = _blend(_xgb("h"),   stat_h_rate)
    base_r,   src_r   = _blend(_xgb("r"),   stat_r_rate)
    base_rbi, src_rbi = _blend(_xgb("rbi"), stat_rbi_rate)
    ...
    # ── Confidence tier ───────────────────────────────────────────────────
    n_models = sum(1 for m in xgb_models.values() if m is not None)
    if n_models >= 3 and season_pa >= 100 and sc_conf > 0.4:
        tier = "High"
    elif season_pa >= 60 or sc_conf > 0.2 or n_models >= 1:
        tier = "Medium"
    else:
        tier = "Low"
    ...
    return {
        ...
        "statcast_metrics": {
            ...
            if k in ("barrel_pct","barrel_pa","hard_hit_pct","exit_velocity","n_batted_balls","xwoba","xslg")
        },
    }
